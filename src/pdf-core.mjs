...
function addAppendixPages(pdfDoc, font, sections) {
  ...
  const newPage = () => {
    ...
    pageCount += 1;
    page.drawRectangle({
      x: 0, y: firstPageSize.height - 42, width: firstPageSize.width, height: 42, color: HEADER_COLOR,
    });
    page.drawText('キャラクターシート付録', {
      x: margin, y: firstPageSize.height - 28, size: 14, font, color: rgb(1, 1, 1),
    });
  ...
  for (const section of populated) {
    ...
    y -= 6;
    page.drawLine({
      start: { x: margin, y }, end: { x: firstPageSize.width - margin, y }, thickness: 0.7, color: MUTED_COLOR,
    });
    y -= 14;
    for (const line of wrapText(section.body, font, bodySize, contentWidth)) {
...
async function embedPortrait(pdfDoc, form, portraitBytes) {
  if (!portraitBytes) return;
  const bytes = portraitBytes instanceof Uint8Array ? portraitBytes : new Uint8Array(portraitBytes);
  if (!isPng(bytes) && !isJpeg(bytes)) throw new Error('立ち絵はPNGまたはJPEGを指定してください');
  const image = isPng(bytes) ? await pdfDoc.embedPng(bytes) : await pdfDoc.embedJpg(bytes);
  form.getButton('portrait').setImage(image, ImageAlignment.Center);
}
...
export async function generateCharacterPdfFromAssets(character, options) {
  const {
    sourcePdfBytes,
    fontBytes,
    layoutConfig,
  ...
  const template = await createNormalizedTemplateFromAssets({ sourcePdfBytes, fontBytes, layoutConfig });
  const pdfDoc = await PDFDocument.load(template.bytes, { updateMetadata: false });
  pdfDoc.registerFontkit(fontkit);
  const formFont = await pdfDoc.embedFont(fontBytes, { subset: false, features: { locl: false } });
  ...
  for (const skill of layoutConfig.fixedSkills) {
  ...
  character.skills.forEach((skill, skillIndex) => {
    const fixed = aliases.get(skill.normalizedName) ?? layoutConfig.fixedSkills.find((candidate) => (
  ...
  for (const fixed of layoutConfig.fixedSkills) {
  ...
  const customCapacity = layoutConfig.customSkillLabels.length;
  ...
  character.weapons.slice(0, layoutConfig.weapons.count).forEach((weapon, index) => {
    for (const [part, value] of Object.entries(weaponFieldValues(weapon))) {
  ...
  if (character.weapons.length > layoutConfig.weapons.count) {
    appendixOverflow.push(
      `未配置の武器\n${character.weapons.slice(layoutConfig.weapons.count).map((weapon) => JSON.stringify(weapon)).join('\n')}`,
    );
    warnings.push(`武器が${character.weapons.length - layoutConfig.weapons.count}件、フォーム欄を超えたため付録へ移しました`);
  }

  character.equipment.slice(0, layoutConfig.equipmentSources.length).forEach((item, index) => {
    setField(`equipment.${index}`, equipmentSummary(item), `装備${index + 1}`);
  });
  if (character.equipment.length > layoutConfig.equipmentSources.length) {
    appendixOverflow.push(
      `未配置の装備\n${character.equipment.slice(layoutConfig.equipmentSources.length).map(equipmentSummary).join('\n')}`,
    );
    warnings.push(`装備が${character.equipment.length - layoutConfig.equipmentSources.length}件、フォーム欄を超えたため付録へ移しました`);
  }
  ...
  for (const item of layoutConfig.backstory) setField(item.id, character.backstory[item.label] ?? '', item.label);
